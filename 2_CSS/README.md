...
메인축의 기준은 flex-dircetion을 기준으로 잡음 row / column 따라서

- flex-start : 아이템들을 시작점에 정렬한다 (기본값)
- flex-end : 아이템들을 끝점으로 정렬한다
- center : 아이템들을 가운데로 정렬한다
- space-between : 아이템들 사이에 균일한 간격으로 정렬한다
- space-around : 각 아이템의 좌우에 균일한 간격으로 정렬한다
- space-evenly : 아이템들의 사이와 양 끝에 균일한 간격으로 정렬한다
...
- stretch : 아이템들을 끝까지 늘림 (기본값)
- flex-start : 아이템들을 위쪽으로 정렬
- flex-end : 아이템을 아래쪽으로 정렬
- center : 아이템들을 가운데 정렬
...
- 여러 행 정렬
- `flex-wrap: wrap;` 설정된 상태에서,(줄바꿈 허용)
...
- flex-grow와 쌍을 이루는 속성으로,
  flex-basis 값보다 작아질 수 있는지를 결정하는 속성
...
- item들의 순서를 바꿀 수 있는 속성
  숫자가 커질수록 맨 뒤로감 (음수도 가능, 기본값은 모두 0)
